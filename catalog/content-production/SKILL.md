...
- 既存メディア (実行環境のメディアライブラリ) / AI 生成、いずれの経路でも、**最終形は Markdown + `public/images/` の二元構成**に揃える。
...
3. **画像調達の最適化**: 既存メディア (実行環境のメディアライブラリ) と AI 生成の 2 手段を適切に使い分ける
...
- **調達判断**: 既存メディア (実行環境のメディアライブラリ) / AI 生成 の 2 手段
...
|---|---|---|---|
| **既存メディア** (実行環境のメディアライブラリ) | 実物・実在が必要 (施工事例・スタッフ・店舗・症例・物件・商品) | 権利クリア、即座に使用可 | 素材が無い場合はユーザー依頼で停止 |
| **AI生成** | 抽象的イメージ・背景・アイコン・イラスト | 低コスト、自由度高 | 実物 NG、商用利用の権利注意 |

**禁止**: 外部ストックフォト (Unsplash / iStock / Pexels / 写真AC / Pixta 等) の URL を prompt 内で書き出さない。実行環境に存在しない URL は幻覚として扱われる。素材が無い場合は AI 生成に切り替えるか、実物が必要ならユーザーに「メディアライブラリにアップロードしてください」と依頼して停止する。
...
これらは**必ず実写** (実行環境のメディアライブラリ) で対応。無ければユーザーに「実物写真をメディアライブラリにアップロードしてください」と依頼して停止する。
...
1. **調達方法の適材適所**: 2 手段 (既存メディア / AI 生成) を目的別に使い分け
...
| `references/image-sourcing-framework.md` | 画像調達判断フロー (2 手段: 既存メディア / AI 生成) |
